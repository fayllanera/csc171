...
class Gui:
    def __init__(self):
        ...
        self.board_img = pygame.image.load(os.path.join(
            "img", "board.bmp")).convert()
        self.black_img = pygame.image.load(os.path.join(
            "img", "preta.bmp")).convert()
        self.white_img = pygame.image.load(os.path.join(
            "img", "branca.bmp")).convert()
        self.tip_img = pygame.image.load(os.path.join("img",
                                                      "tip.bmp")).convert()
        self.clear_img = pygame.image.load(os.path.join("img",
                                                        "nada.bmp")).convert()
    ...
    def show_options(self):
        ...
        while True:
            ...
            title = title_fnt.render("Welcome to Othello Game!", True, self.WHITE)
            title_pos = title.get_rect(
                centerx=self.screen.get_width() / 2, centery=60)

            start_txt = self.font.render("PLAY", True, self.WHITE)
            start_pos = start_txt.get_rect(
                centerx=self.screen.get_width() / 2, centery=220)
            player1_txt = self.font.render("Choose Player 1", True, self.WHITE)
            player1_pos = player1_txt.get_rect(
                centerx=self.screen.get_width() / 2, centery=260)
            player2_txt = self.font.render("Choose Player 2", True, self.WHITE)
